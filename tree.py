    ...
    for i in range(len(geo)):
        ...
        if feature is not None:
            ...
            if feature["geometry"]["type"] == "Polygon":
                coords_len = len(feature["geometry"]["coordinates"][0])
                coords = (
                    np.array(
                        [
                            np.array(x).astype(np.float64)
                            for x in feature["geometry"]["coordinates"][0]
                        ]
                    )
                    .astype(np.float64)
                ...
                # df.at[i, "type"] = "Polygon"
                # df.at[i, "geometry"] = coords
                df.at[i, "bbox"] = np.array(get_bbox(coords))
                # df.at[i, "centroid"] = str(np.array(get_centroid(coords)))
print(df)

@njit
def make_poly_arr(max_len_polys, coords):
    poly_arr = coords

    # Working out the final array
    poly_arr_len = poly_arr.shape[0]
    remaining_len = max_len_polys - poly_arr_len

    # Add zero arrays to get max len of all polygons to make all arrays
    # of equal length
    if max_len_polys > poly_arr_len:
        zero_add = np.zeros((remaining_len, 2)).astype(np.float64)
        final_poly_arr = np.concatenate((poly_arr, zero_add), axis=0)
    else:
        final_poly_arr = poly_arr

    return final_poly_arr, poly_arr
...
with fiona.open(geo_file) as geo:
    ...
    polys = np.empty((len(geo), max(length_polygons), 2), dtype=np.float64)
    max_len_polys = max(length_polygons)
    bboxes = np.empty((len(geo), 4))
    centroids = np.empty((len(geo), 2, 2))
...
coords = np.array(coords).astype(np.float64).reshape(len(coords), 2)

                final_poly_arr, poly_arr = make_poly_arr(max_len_polys, coords)

                # # Converting and calculating all necessary data
                polys[i] = final_poly_arr
                bboxes[i] = get_bbox(poly_arr)
                centroids[i] = get_centroid(poly_arr)
t2 = timeit.default_timer() - t1
print(t2)
for i in prange(len(polys)):
    p = polys[i]
    b = bboxes[i]
    c = centroids[i]
    if not b.all() == 0:
        # Clean up
        p = [x for x in p if x.all() != 0]
        if len(p) > 0:
            p = np.array(p)
            print(p, b, c)
            print("-" * 20)
...
# src = list(zip(polys, bboxes, centroids))
# data_arr = pd.DataFrame(src, columns=["polygons", "bboxes", "centroids"]).to_numpy()

src = list(zip(polys, bboxes))
data_arr = pd.DataFrame(src, columns=["polygons", "bboxes"]).to_numpy()
print(data_arr[0][1])
# if len(polys) == len(bboxes) == len(centroids):
#     for i in range(len(polys)):
#         print("POLYGON: ", polys[i])
#         print("BBOX: ", bboxes[i])
