...
def SortByAngle(kNearestPoints, currentPoint, prevPoint):
    ...
    i = 0
    for x in range(len(kNearestPoints)):
        NearestPoint = kNearestPoints[x]
        # calculate the angle
        angle = np.arctan2(
...
points_intersect_solution = np.array(
    [[1, 1], [10, 3], [11, 8], [9, 14], [15, 21], [-5, 15], [-3, 10], [1, 1]]
)
